...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             */
            val acquired1 : AcquiredAccount?
            val acquired2 : AcquiredAccount?
            if (fromIndex <= toIndex) {
                acquired1 = acquire(fromIndex, this)
                acquired2 = acquire(toIndex, this)
            } else {
                acquired2 = acquire(toIndex, this)
                acquired1 = acquire(fromIndex, this)
            }

            if (acquired1 != null && acquired2 != null) {
                when {
                    amount > acquired1.amount -> errorMessage = "Underflow"
                    acquired2.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        acquired1.newAmount = acquired1.amount - amount
                        acquired2.newAmount = acquired2.amount + amount
                    }
                }
                completed = true
            }
